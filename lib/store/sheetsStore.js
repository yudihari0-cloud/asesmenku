...
// CATATAN: kolom baru SELALU ditambahkan di UJUNG — menyisipkan di tengah
// akan menggeser arti kolom pada baris lama (data saling tertukar).
export const SHEETS_SCHEMA = {
  users: ['id', 'role', 'nama', 'username', 'nisn', 'kelasId', 'sekolahId', 'namaSekolah', 'wa', 'nip', 'jabatan', 'foto', 'pwHash', 'createdAt', 'admin'],
  sekolah: ['id', 'nama', 'alamat', 'logo', 'createdAt', 'masaAktifSampai'],
  aktivasi: ['id', 'kode', 'status', 'dibuatUntuk', 'dipakaiSekolahId', 'dipakaiOleh', 'activatedAt', 'kedaluwarsa', 'createdAt'],
...
async function gapi(path, init) {
  ...
  try {
    ...
    const mentah = String(process.env.GOOGLE_SHEETS_ID || '').slice(0, 120);
    throw new Error(
      'Google API non-JSON. status=' + res.status + ' finalUrl="' + finalUrl + '" sheetId="' + dipakai + '" env="' + mentah + '" body: ' + text.slice(0, 400)
    );
  }
...
export const sheetsStore = {
  ...
  async insert(col, doc) {
    ...
    const url = `/values/${encodeURIComponent(col + '!A1')}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`;
    const init = {
      method: 'POST',
    ...
    try {
      ...
      if (/unable to parse range/i.test(e.message)) {
        // tab belum ada — buat lalu coba sekali lagi
        await ensureTab(col);
        await gapi(url, init);
      } else {
        throw e;
