...
export async function boot() {
  if (!seeded) {
    ...
    try {
      ...
      // Migrasi 2: akun yudi = admin
      const yudi = users.find((u) => u.username === 'yudi' && u.role === 'guru');
      if (yudi && !yudi.admin) await store.update('users', yudi.id, { admin: true });

      // Migrasi 3: sekolah lama tanpa masa aktif -> demo 10 tahun, lainnya 1 tahun
      const sekolahs = await store.list('sekolah');
      for (const s of sekolahs) {
        if (!s.masaAktifSampai) {
